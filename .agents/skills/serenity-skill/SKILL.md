---
name: serenity-skill
description: "Use only for source-backed intelligence research and supply-chain bottleneck research in daily_stock_analysis: industry-chain decomposition, scarce-layer or bottleneck discovery, purple-leaf/bottleneck candidate mining, public filing/announcement/customer/capacity/price-increase/risk verification, and research candidate lists. Trigger on explicit Serenity, 产业链/供应链/卡点/瓶颈, 紫苏叶, 候选清单, or thesis stress-test requests. Research support only; never for buy/sell instructions, intraday trade approval, pre-market trade plans, broker access, or trading-discipline rule changes."
license: MIT
metadata:
...
## daily_stock_analysis project boundary

This is a project-level research skill for the current stock-intelligence research system. It must stay isolated from the trading-discipline system.

Allowed uses:

- industry-chain decomposition;
- supply-chain bottleneck identification;
- 紫苏叶 / bottleneck candidate discovery;
- public announcement, filing, customer, capacity, price-increase, and risk verification;
- research candidate list output.

Forbidden uses:

- outputting direct buy or sell instructions;
- temporarily approving or relaxing intraday trading restrictions;
- modifying trading-discipline system rules;
- automatically generating pre-market trading plans;
- accessing brokers, wallets, accounts, local secrets, or execution systems;
- writing to trading-discipline files, strategy rules, order templates, broker configuration, or global Codex rules.

When a user asks a trading-adjacent question, convert it into research priority language. Use wording such as:

`我只能输出产业链瓶颈研究候选清单和核验路径，不能给出买入/卖出指令或放行交易。`

If a file output is requested, write only to:

`outputs/serenity_candidates/YYYY-MM-DD_主题_候选清单.md`

The output file must be a research candidate list, not a trading plan.
